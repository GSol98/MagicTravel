...
import Servizi from "./ServiziCasaVacanze.js";
...
import CostoMensile from "./Costo.js";
...
class FormContainerCasaVacanza extends React.Component {
    ...
    handleSubmit = event => {
        ...
        document.getElementById('bottoneCreaAnnuncioCasa').disabled = true;

        let id = decripta(window.sessionStorage.getItem("id_account"), window.sessionStorage.getItem("email"))
        // Inserisco l'annuncio
        fetch("/inserimento_annunci/crea_annuncio_casa_vacanze", {
            method: "POST",
            body: JSON.stringify({
                id_proprietario: id,
                state: this.state
        ...
        .then(res => {
            if (res.status === 200) {
                ...
                fetch("inserimento_annunci/inserimento_immagine_copertina", {
                    ...
                    .then(res => {
                        if (res.status === 200) {
                            // Inserisco tutt le altre immagini
        ...
        .catch(err => {
            ...
            document.getElementById('bottoneCreaAnnuncioCasa').disabled = false;
    ...
    render() {
        return (
            <form className="container was-validated" id="Annuncio" onSubmit={this.handleSubmit}>
                ...
                <button style={{ float: "right" }} className="btn btn-secondary my-5" id='bottoneCreaAnnuncioCasa'>Crea Annuncio</button>
                <br /><br /><br />
            </form>
        )
    }
