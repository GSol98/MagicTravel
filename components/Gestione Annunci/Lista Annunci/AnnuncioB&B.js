...
const Annuncio = props => {
    let min = 9999;
    let max = 0;
    props.stanze.forEach(stanza => {
        if (stanza.prezzo > max)
            max = stanza.prezzo;
        if (stanza.prezzo < min)
            min = stanza.prezzo
    });

    return (
        <div className="my-5 p-3 border rounded shadow-lg">
            <a className="text-dark text-decoration-none" href={`/DettagliAnnuncio#B${props.annuncio.id_annuncio}`}>
                <div className='row'>
                    ...
                    <div className="p-3 col-md-5 col-sm-12 col-12 testo">
                        ...
                        <p style={{ fontSize: "1.2em", marginBottom: '-3px' }}>Prezzo: {min !== max ? <span>da <b>{min}€</b> a <b>{max}€</b></span> : <span><b>{min}€</b>/notte</span>}</p>
                        <p>*Tasse escluse</p>
                    </div>

                    <div className="col-md-3 col-sm-12 col-12p-3 text-center">
